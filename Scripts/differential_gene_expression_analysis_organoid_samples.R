...
#combine organoids from healthy tissue independent of diagnosis of the patient for this analysis
metadata[metadata$Part == "Normal", ]$Group <- "control"
metadata <- metadata %>%
...
#prepare metadata
sample_info_filtered <- subset(metadata, metadata$Group %in% c("control", "insulinoma"))

#import count data
files <- paste("count_files_organoid_samples/salmon_files/", sample_info_filtered$Sample_ID, "_quant.sf", sep = '')
...
#run DE analysis
dds_counts <- DESeqDataSetFromTximport(txi, sample_info_filtered, ~ Group)
...
write.table(res_sorted, file = paste0(output_directory, "DEres_organoid_samples_Insulinoma_Normal_vs_Tumor.txt"), sep = "\t", quote = FALSE)





## Volcano plot for insulinoma DEGs as in Supplementary Figure 3A ------------------------------------------------------------

#load DGE analysis results and filter for DEGs
res_table_insulinoma <- read.csv("results/DGE_analysis/output/DEres_organoid_samples_Insulinoma_Normal_vs_Tumor.txt", sep = '\t')
...
label_fc <- df_plot %>% 
  ...
  {c(head(., 5)$Gene_ID, tail(., 5)$Gene_ID)}
...
pdf("results/DGE_analysis/output/volcanoplot_insulinoma_organoid_samples.pdf", width = 8, height = 5)
